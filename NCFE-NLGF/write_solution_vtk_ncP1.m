% Write a .vtk file readable by paraview
%
function out=write_solution_vtk_ncP1(u,namefile,ncell,nedge,nvert,cell_v,cell_n,cell_e,vertex)

% First reconstruct cell values and put them at the start of the vector
utot=zeros(ncell+nedge,1);
utot(ncell+1:ncell+nedge)=u;
for i=1:ncell
	for j=1:3
		jj=cell_e{i}(j);
		utot(i) = utot(i) + u(jj)/3;
	end
end

% Then compute values at the vertices
