function [U, num_updates, iter, res] = compute_staionary_system(cell_v, cell_e, ncell, nedge, vertex, area, mpe, dt, idt, imesh, epsilon, itermax, tol, relaxmin, num_updates, M, b, U_pre);
...
%% Newton
X_pre = U_pre;% initial guess of the solution
% X_pre = test_cases(idt*dt,mpe, 1)'; % initial guess of the solution
relax = 1;
res_pre = 1;
while ( iter < itermax && res > tol && relax > relaxmin )

    [J, F] = assemble_jacobian_system(cell_v, cell_e, ncell, vertex, nedge, area, mpe, U_pre,epsilon);

    Aglob = M + dt .* J;
    RHS = rhs - (M + dt .* F)* X_pre;

    % isequal_numeric = (norm(Fu) == norm(RHS))
