...
for imesh=1:nbmeshes
    ...
    for idt = 1 : Ndt(imesh)
        ...
        [L2error(idt), W11error(idt), H1error(idt)] = compute_norms(U - test_cases(idt * dt, vertex, ucase)', area, vertex, cell_v);
        [L2norm(idt), W11norm(idt), H1norm(idt)] = compute_norms(test_cases(idt * dt, vertex, ucase)', area, vertex, cell_v);
...
legend('CP1FEM','Location','southeast');
% grid on
subplot(1,3,2);
loglog(h,L1W11error,'.-');
axis([5e-3 5e-1 1e-3 7e-1]);
...
xlabel('$h$','Interpreter','latex')
% ylabel('$\left\| \nabla u-\nabla \overline{u} \right\|_{L^2(L^2)}$','Interpreter','latex')
legend('CP1FEM','Location','southeast')
% hold on
% grid on
